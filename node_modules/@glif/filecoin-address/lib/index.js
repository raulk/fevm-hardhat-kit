...
var base32 = base32Function('abcdefghijklmnopqrstuvwxyz234567'); // Store valid CoinTypes / Protocols for runtime validation

var coinTypes = Object.values(CoinType);
var protocols = Object.values(Protocol).filter(function (p) {
...
var namespaceByteLen = new Int64(0).toBuffer().length; // The maximum length of `int64` as a string.

var maxInt64StringLength = 19; // The hash length used for calculating address checksums.

var checksumHashLength = 4;
...
export function newDelegatedAddress(namespace, subAddr, coinType) {
  ...
  var namespaceBuf = new Int64(namespace).toBuffer();
    ...
    case Protocol.DELEGATED:
      {
        ...
        var namespaceBuf = new Int64(namespaceNumber).toBuffer();
    ...
    case Protocol.BLS:
      {
        ...
        if (protocol === Protocol.SECP256K1 || protocol === Protocol.ACTOR) if (_payload2.length !== payloadHashLength) throw Error('Invalid address length');
        if (protocol === Protocol.BLS) if (_payload2.length !== blsPublicKeyBytes) throw Error('Invalid address length');
...
export function delegatedFromEthAddress(ethAddr) {
  var coinType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : CoinType.TEST;
...
export function ethAddressFromDelegated(delegated) {
  return utils.getAddress("0x".concat(decode(delegated).subAddrHex));
}
export default {
  Address: Address,
